...
Phase 2:
- post_feed / post_story ジョブを Instagrapi 経由で実行
...
def _run_loop(client: redis.Redis, config: WorkerConfig) -> None:
    while True:
        ...
        result = handle(job, config=config)
        push_result(client, result)


def handle(job: JobPayload, *, config: WorkerConfig, ig_factory=None) -> JobResult:
